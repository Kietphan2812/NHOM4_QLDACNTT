...
Write-Host " SERVER IS RUNNING ON http://localhost:3000" -ForegroundColor Cyan
...
function Execute-SqlQuery([string]$query) {
    try {
        ...
        Write-Host "SQL Query Error: $_" -ForegroundColor Red
        return "[]"
    }
}
...
function Execute-SqlNonQuery([string]$query) {
    try {
        ...
        Write-Host "SQL Execute Error: $_" -ForegroundColor Red
        return $false
    }
}
